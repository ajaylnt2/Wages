class ChillerPerformanceController {
  constructor($rootScope, $scope, auth, $timeout, $location,myService, performanceService) {
    ...
    this.currentMonth = d.toDateString().substr(4, 3);
    this.currentDay = d.toDateString().substr(8, 2);
    this.noDays =new Date(d.getFullYear(), d.getMonth()+1, 0).getDate();
    this.totalDays = [];
      for(var i=1; i<=this.noDays; i++)
      {
        this.totalDays[i]=i.toString();
      }
...
disabledDaySelect()
{
  ...
  init(data, color, status) {
    ...
    var obj = this;
    this.performanceService.getChillerPerformance()
      .then(function(data) {
  ...
  //DropDown Begins
   getAnnualDetails(data) {
    ...
    this.totalDays.shift();
      this.totalDays.unshift('All');
    this.yearProperties = {
    ...
    this.monthProperties = {
      placeHolder: 'All Options',
      model: this.currentMonth.toString(),
      label: 'Monthly',
      options:this.months,
      onChange: () => {
      },
    ...
    this.dayProperties = {
      placeHolder: 'All Options',
      model: this.currentDay.toString(),
      label: 'Daily',
      options:this.totalDays,
      onChange: () => {
      },
    };
  ...
  runLineChart(color, status) {
    // this.yearlyData("All");
    this.dailyData(this.currentYear.toString(), this.currentMonth.toString(), this.currentDay.toString());
  }
  //Line Chart Ends
...
export default ChillerPerformanceController;
