...
let context = require.context('./client/components/controlElements', true, /\.spec\.js/);
let app = require.context('./client/app', true, /\.spec\.js/);
let dialog = require.context('./client/components/dialog', true, /\.spec\.js/);
let listingelements= require.context('./client/components/listingElements', true, /\.spec\.js/);
let primitivelements= require.context('./client/components/primitiveElements', true, /\.spec\.js/);
let reusablecharts= require.context('./client/components/reusableCharts', true, /\.spec\.js/);
...
dialog.keys().forEach(dialog);
listingelements.keys().forEach(listingelements);
primitivelements.keys().forEach(primitivelements);
reusablecharts.keys().forEach(reusablecharts);
